    ...
    <>
      ...
      >
        ...
        <section className='mt-8 px-6'>
          ...
          <p className='text-[#4B5565] text-sm font-normal mb-5'>{t('Filter results to facilitate access to the required service')}</p>
        </section>
        <span className='border-[0.5px] border-[#E3E8EF]  '/>

        <section className='border m-6 p-6 border-[#CDD5DF] rounded-[5px]'>
          <p className='mb-6 text-[#364152] text-xl font- font-medium'>{t('Services status')}</p>
          <div>
            ...
            <label key={index} className="flex items-center gap-2 mb-4 cursor-pointer">
              ...
              <span
                className={`w-6 h-6 flex items-center justify-center border rounded-[3px] 
                  ${selected.includes(option) ? "bg-[#C69815] border-[#C69815]" : "bg-white border-[#CDD5DF]"}`}
              >
                {selected.includes(option) && (
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path d="M13 4.25L6.125 11.125L3 8" stroke="white" strokeWidth="2.08325" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                )}
              </span>
        ...
        <div className='px-6 mt-4 mb-6 flex gap-4'>
          <button className=' bg-[#C69815] text-white px-4 py-2.5 w-42.5 h-13.5 rounded-[3px]'>{t('apply')}</button>
          <button className='border border-[#C69815] text-[#C69815] px-4 py-2.5 w-32.5 h-13.5 rounded-[3px]'
